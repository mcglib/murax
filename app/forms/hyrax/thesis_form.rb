...
module Hyrax
  ...
  class ThesisForm < Hyrax::Forms::WorkForm
    ...
    self.required_fields += [:nested_ordered_creator,:date, :rights, :rtype, :department, :subject, :degree, :institution, :publisher]
    self.required_fields -= [:keyword, :contact_email, :description, :faculty, :rights_statement]
    self.single_valued_fields = [:title, :rtype]

    def primary_terms
      [:title, :nested_ordered_creator,:date, :rights, :rtype, :department, :subject, :degree, :institution, :publisher ] | super
    end
