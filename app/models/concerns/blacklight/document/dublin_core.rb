  ...
  def export_as_oai_dc_xml
    ...
    xml.tag!("oai_dc:dc",
      ...
      to_semantic_values.select { |field, _values| dublin_core_field_name? field  }.each do |field, values|
        # oai engine uses the base_url configured in oai_config.rb to build the identifier, instead of the base url of the repository
        if field.to_s.include? 'identifier'
          Array.wrap(values).each do |v|
             xml.tag! "dc:identifier", v.gsub(/catalog\/oai\//,'')
          end
          next
        end
        Array.wrap(values).each do |v|
