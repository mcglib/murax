namespace :report do
  desc 'Output work ids and DigiTool PIDs for a specified Samvera collection'
  task :report_work_ids_with_pids_by_collection_id, [:collection] => :environment do |t,args|
    if args.count == 0
      puts "Usage: bundle exec rake report:report_work_ids_with_pids_by_collection_id['samvera_collection_id']"
