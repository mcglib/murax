...
namespace :murax do
  ...
  task :export_metadata_records_as_dc_xml, [:destination] => :environment do |task, args|
    dest = args.destination
    workids = args.extras
    if dest.empty? or workids.empty?
      puts "Usage: bundle exec rake murax:export_metadata_records_as_dc_xml['destination-of-output','work_id'[,'work-id'...]]"
      puts "       Exports the metadata for the specified record(s) as Dublin Core xml"
    ...
    workids.each do |work_id|
     ...
     noko_xsl = Nokogiri::XSLT(File.read("#{Rails.root}/lib/tasks/murax/samvera2DC.xsl"))
